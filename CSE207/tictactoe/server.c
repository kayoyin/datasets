...
int check_winner(Position** grid, int npos){
  ...
  for (int i = (int)lplay - 1; i < npos+1; i = i+2){
    // iterate over positions current player occupies
    // players are alternating so we can increment index by 2
    row = grid[i]->row;
    col = grid[i]->col;
    ...
    if ((int)lrow + (int)lcol == 4){ // (1,3) (2,2) or (3,1)
